import type { Competition, Event, EventId, Person } from "@wca/helpers";
import { useState } from "react";
import { fetchWCIF } from "@/services/wca/official";
import { getCompetitionsFromNow } from "@/services/wca/unofficial";
...
export function useStatistics() {
  ...
  const generateStatistics = async () => {
    try {
      ...
      const compData = sorted.map((comp: Competition) => {
        ...
        const closed =
          new Date(comp.registrationInfo.closeTime) < new Date()
            ? " (Closed)"
            : "";
        return {
          Name: comp.name,
      ...
      const rounds = sorted.map((comp) => {
        ...
        const totalRounds = Object.values(eventData).reduce(
          (a: number, c) => a + (Number(c) || 0),
          0,
        );
        const totalEvents = Object.values(eventData).filter(
          (v) => v !== "",
        ).length;
        return {
          Name: comp.name,
      ...
      const withReg = comps.filter((c) =>
        c.persons.some((p: Person) => p.registration),
      );
      const signups = withReg.map((comp) => {
        ...
        for (const ev of EVENTS) {
          const count = regged.filter((p: Person) =>
            p.registration?.eventIds.includes(ev.id as EventId),
          ).length;
          const info = comp.events.find((e: Event) => e.id === ev.id);
          const pct = Math.round((count / regged.length) * 100)
            .toString()
            .padStart(2, "0");
