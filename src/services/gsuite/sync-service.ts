...
function parseTeamConfigFromGrid(
  sheet: SheetWithData | undefined,
): TeamConfig[] {
...
export async function loadSheetData(
  ...
  const rolesDbSheet = spreadsheet.sheets.find(
    (s) => s.properties.title === ROLES_DB_SHEET,
  );
...
export async function generateSyncPreview(
  ...
  try {
    const { memberships: desiredMemberships } = await loadSheetData(
      accessToken,
      spreadsheetId,
    );
    ...
    for (const group of allGroups) {
      try {
        ...
        preview.errors.push(
          `Failed to fetch members for ${group.email}: ${getErrorMessage(error)}`,
        );
      }
    }
    ...
    preview.errors.push(
      `Failed to generate preview: ${getErrorMessage(error)}`,
    );
  }
  ...
  if (options.additions) {
    for (const change of preview.additions) {
      try {
        ...
        result.errors.push(
          `Failed to add ${change.memberEmail} to ${change.groupEmail}: ${getErrorMessage(error)}`,
        );
      }
    }
  }
  ...
  if (options.removals) {
    for (const change of preview.removals) {
      try {
        ...
        result.errors.push(
          `Failed to remove ${change.memberEmail} from ${change.groupEmail}: ${getErrorMessage(error)}`,
        );
      }
    }
  }
  ...
  if (options.updates) {
    for (const change of preview.updates) {
      try {
        ...
        result.errors.push(
          `Failed to update ${change.memberEmail} in ${change.groupEmail}: ${getErrorMessage(error)}`,
        );
      }
    }
  }
