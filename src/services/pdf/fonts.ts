...
export async function embedFont(doc: PDFDocument, name: string): Promise<PDFFont> {
  ...
  if (docFonts.has(name)) return docFonts.get(name)!
...
export async function preloadFonts(doc: PDFDocument): Promise<Record<string, PDFFont>> {
