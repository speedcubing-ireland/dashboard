...
type ScheduleColumn = 'time' | 'event' | 'stage' | 'group' | 'station' | 'staff'
...
function getColumnWidths(config: BadgeConfig, totalWidth: number): Record<string, number> {
...
function getFillColor(config: BadgeConfig, assignment: AssignmentInfo, alt: number): [number, number, number] {
...
function buildRoleText(a: AssignmentInfo): string {
  const parts: string[] = []
  const short = (a.judging.length ? 1 : 0) + (a.running.length ? 1 : 0) + (a.scrambling.length ? 1 : 0) > 1
  if (a.judging.length) parts.push(`${short ? 'J' : 'Judge'}: ${a.judging.join(', ')}`)
  if (a.running.length) parts.push(`${short ? 'R' : 'Run'}: ${a.running.join(', ')}`)
  if (a.scrambling.length) parts.push(`${short ? 'S' : 'Scram'}: ${a.scrambling.join(', ')}`)
  return parts.join(' ')
}

async function drawEventIcon(ctx: BadgeContext, page: PDFPage, code: string, fontSize: number, x: number, y: number, scale = 1): Promise<number> {
...
async function drawTextBox(ctx: BadgeContext, page: PDFPage, text: string, opts: TextBoxOptions, pageHeight: number): Promise<void> {
  ...
  page.drawRectangle({
    x: mmToPoints(x), y: boxY - boxH, width: mmToPoints(w), height: boxH,
    color: rgbColor(fillColor[0], fillColor[1], fillColor[2]),
    borderColor: rgb(0.5, 0.5, 0.5), borderWidth: 0.1,
  })
  ...
  const iconWidth = eventCode ? await (async () => {
     try {
       const iconFont = await embedFont(ctx.doc, 'cubing-icons')
       const s = fontSize * LAYOUT.iconSizeRatio
       return iconFont.widthOfTextAtSize(EVENT_ICON_MAP[eventCode] || '', s) || s
     } catch { return 0 }
  })() : 0

  const iconSpacing = iconWidth > 0 ? fontSize * LAYOUT.iconSpacingRatio : 0
  const availableWidth = w - (iconWidth > 0 ? (iconWidth + iconSpacing) * 25.4 / 72 + xPad : xPad)
  const hScale = opts.scale ?? Math.min(1, mmToPoints(availableWidth) / textWidth)
  const scaledSize = fontSize * hScale

  let textX = 0, iconX = 0
  if (align === 'center') {
    const total = iconWidth * hScale + iconSpacing * hScale + getTextWidth(text, regularFont, scaledSize)
    const center = mmToPoints(x + w / 2)
    iconX = center - total / 2
    textX = iconX + (iconWidth > 0 ? iconWidth * hScale + iconSpacing * hScale : 0)
  } else {
    iconX = mmToPoints(x + xPad)
    textX = iconX + (iconWidth > 0 ? iconWidth * hScale + iconSpacing * hScale : 0)
  }

  if (eventCode) await drawEventIcon(ctx, page, eventCode, fontSize, iconX, textY, hScale)
  page.drawText(text, { x: textX, y: textY, size: scaledSize, font: regularFont })
}

async function drawName(ctx: BadgeContext, page: PDFPage, text: string, align: 'left' | 'center', x: number, y: number, w: number, h: number, fontName = 'NotoSans-Bold'): Promise<void> {
  ...
  if (localName && ctx.config.includeLocalNames) {
    ...
    let baseX = align === 'center' ? mmToPoints(x + w / 2) - total * scale / 2 : mmToPoints(x)
    ...
    const drawX = align === 'center' ? mmToPoints(x + w / 2) - scaledWidth / 2 : mmToPoints(x)
    page.drawText(latinName, { x: drawX, y, size: scaled, font })
  }
}

async function splitNameIntoLines(ctx: BadgeContext, text: string, fontSize: number): Promise<[string, string]> {
  ...
  const lengths = parts.map((p) => {
    ...
    if (m && ctx.config.includeLocalNames) {
      ...
      return Promise.resolve(localFont).then((lf) =>
...
async function drawNameSide(ctx: BadgeContext, page: PDFPage, info: PersonScheduleInfo, offsetX: number, offsetY: number, width: number, height: number, pageHeight: number): Promise<void> {
  if (ctx.backgroundImage) {
    ...
    page.drawImage(ctx.backgroundImage, { x: drawX, y: drawY, width: drawW, height: drawH })
  }

  if (!info.blank) {
    const lines = await splitNameIntoLines(ctx, info.name, 10 * LAYOUT.fontSizeMultiplier)
    const lineSpacing = mmToPoints(9)
    const y1 = flipY(pageHeight, offsetY + 71)
    await drawName(ctx, page, lines[0], 'center', offsetX + 3, y1, width - 6, 10)
    await drawName(ctx, page, lines[1], 'center', offsetX + 3, y1 - lineSpacing, width - 6, 10)
  }

  page.drawLine({
    start: { x: mmToPoints(offsetX + 5), y: flipY(pageHeight, offsetY + 83) },
    end: { x: mmToPoints(offsetX + width - 5), y: flipY(pageHeight, offsetY + 83) },
    thickness: mmToPoints(0.25), color: rgb(0, 0, 0),
  })
  ...
  if (!info.blank) {
    ...
    page.drawText(idText, {
      ...
      y: flipY(pageHeight, offsetY + 88), size: 13, font: regularFont, color,
    })
  }
  ...
  if (ctx.wcaLogoImage) {
    ...
    page.drawImage(ctx.wcaLogoImage, { x: mmToPoints(offsetX + 3), y: logoY, width: mmToPoints(10 * r), height: logoH })
  }

  if (ctx.logoImage) {
    const r = ctx.logoImage.width / ctx.logoImage.height
    page.drawImage(ctx.logoImage, { x: mmToPoints(offsetX + width - 10 * r - 3), y: logoY, width: mmToPoints(10 * r), height: logoH })
  }
  ...
  if (!info.blank) {
    ...
    if (flag) {
      ...
      page.drawImage(flag, {
        ...
        width: mmToPoints(flagW), height: flagH,
      })
      if (!['np', 'tw'].includes(info.countryCode)) {
        page.drawRectangle({
          ...
          y: flipY(pageHeight, offsetY + height - 15) - flagH,
          width: mmToPoints(flagW), height: flagH,
          borderColor: rgb(0, 0, 0), borderWidth: 0.1,
        })
      }
    }
  }
...
function shouldShowAssignment(a: AssignmentInfo, config: BadgeConfig): boolean {
  return !(a.competing === -1 && (!config.includeStaffing || config.hideStaffOnlyAssignments))
}

async function drawSchedule(ctx: BadgeContext, page: PDFPage, x: number, y: number, w: number, info: PersonScheduleInfo, pageHeight: number, maxHeight?: number): Promise<number> {
  if (info.blank) return 0

  const widths = getColumnWidths(ctx.config, w)
  const columns = getVisibleColumns(ctx.config)
  const headerH = 2.5, assignH = 3.5, dayH = 4
  const headerMap: Record<string, string> = { time: 'Time', event: 'Event', stage: 'Stage', group: 'Group', station: 'Station', staff: 'Staff Groups' }

  let row = y
  let col = x
  for (const c of columns) {
    const w = widths[c]
    await drawTextBox(ctx, page, headerMap[c], { rect: { x: col, y: row, w, h: headerH } }, pageHeight)
    col += w
  }
  row += headerH
  ...
  for (const day of info.sortedSchedule) {
    ...
    await drawTextBox(ctx, page, WEEK_DAYS[day.day] || 'Unknown', { rect: { x, y: row, w, h: dayH }, align: 'center' }, pageHeight)
    row += dayH
    ...
    for (const a of day.sortedAssignments) {
      ...
      for (const c of columns) {
        ...
        else if (c === 'event') { text = a.eventText; ev = a.eventCode }
        else if (c === 'stage') text = ctx.config.removeStageWord ? removeStageWord(a.stageText) : a.stageText
        else if (c === 'group') text = a.competing === -1 && ctx.config.includeStaffing ? '-' : `${a.competing}`
        else if (c === 'station') text = a.competing === -1 ? '-' : a.stationNumber === null ? 'any' : `${a.stationNumber}`
        else if (c === 'staff') text = buildRoleText(a)
        
        await drawTextBox(ctx, page, text, { 
...
async function drawScheduleSide(ctx: BadgeContext, page: PDFPage, info: PersonScheduleInfo, offsetX: number, offsetY: number, width: number, height: number, pageHeight: number): Promise<void> {
  const regularFont = await embedFont(ctx.doc, 'NotoSans-Regular')
  await drawName(ctx, page, info.name, 'left', offsetX + 3, flipY(pageHeight, offsetY + 7), width - 12, 4, 'NotoSans-Bold')
  ...
  page.drawText(compidText, {
    ...
    y: flipY(pageHeight, offsetY + 6), size: 7, font: regularFont,
  })

  const startY = offsetY + 10
  const maxH = height - 10 - 5
  const schedH = await drawSchedule(ctx, page, offsetX + 3, startY, width - 6, info, pageHeight, maxH)

  const space = height - 10 - schedH
  if (ctx.config.showWcaLiveQrCode && ctx.qrCodeImage && ctx.config.qrCodeText && space >= 18) {
...
function drawCuttingLines(page: PDFPage): void {
  page.drawLine({
    ...
    thickness: 0.25, color: rgb(0.5, 0.5, 0.5), dashArray: [1],
  })
  page.drawLine({
...
async function embedImages(doc: PDFDocument, images: { background?: Uint8Array; logo?: Uint8Array; wcaLogo?: Uint8Array; flags: Map<string, Uint8Array>; qrCode?: Uint8Array }) {
  const flagImages = new Map<string, PDFImage>()
  const embed = async (bytes?: Uint8Array) => bytes ? doc.embedPng(bytes) : null
  ...
  for (const [code, bytes] of images.flags) {
    try { flagImages.set(code, await doc.embedPng(bytes)) } catch {}
  }

  return { backgroundImage: bg, logoImage: logo, wcaLogoImage: wca, flagImages, qrCodeImage: qr }
}

export async function generateSingleBadge(info: PersonScheduleInfo, config: BadgeConfig, images: { background?: Uint8Array; logo?: Uint8Array; wcaLogo?: Uint8Array; flag?: Uint8Array; qrCode?: Uint8Array }): Promise<PDFDocument> {
...
export async function generateAllBadges(
  ...
  config: BadgeConfig, 
  images: { background?: Uint8Array; logo?: Uint8Array; wcaLogo?: Uint8Array; flags: Map<string, Uint8Array>; qrCode?: Uint8Array },
  ...
  for (let i = 0; i < persons.length; i++) {
    ...
    const offsetY = row * (A4L_HEIGHT / 2) + (A4L_HEIGHT / 2 - A6L_HEIGHT) / 2

    await drawBadge(ctx, page!, persons[i], offsetX, offsetY, mmToPoints(A4L_HEIGHT))
