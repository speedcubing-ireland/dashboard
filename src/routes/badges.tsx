...
import { preloadFlags, extractUrl, generateQRBytes, prepareImages } from '@/services/assets'
...
import { SettingsIcon, ArrowLeft02Icon } from '@hugeicons/core-free-icons'
import { ProgressButton } from '@/components/common/progress-button'

export function BadgesPage() {
  const { wcif, activities, config, error, setWcif, setActivities } = useBadgeStore()
  const [isGenerating, setIsGenerating] = useState(false)
  const [progress, setProgress] = useState(0)
  const [refreshTrigger, setRefreshTrigger] = useState(0)

  const handleReset = () => {
    if (confirm('Are you sure you want to change competition? Current data will be cleared.')) {
      setWcif(null)
      setActivities({})
    }
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        {wcif && (
          <div className="flex items-center gap-2">
            <Button variant="outline" size="icon" onClick={handleReset} title="Change Competition">
              <HugeiconsIcon icon={ArrowLeft02Icon} strokeWidth={2} className="size-4" />
            </Button>
            <Sheet onOpenChange={(open) => !open && setRefreshTrigger(prev => prev + 1)}>
              <SheetTrigger asChild>
                <Button variant="outline">
                  <HugeiconsIcon icon={SettingsIcon} strokeWidth={2} className="mr-2 size-4" />
                  Settings
                </Button>
              </SheetTrigger>
            ...
            <ProgressButton onClick={handleGenerateAll} isProcessing={isGenerating} progress={progress}>
              Generate All Badges
            </ProgressButton>
          </div>
        )}
      ...
      {wcif && (
        <>
          <CompetitionSummary />
          <BadgePreview refreshTrigger={refreshTrigger} />
        </>
      )}
    </div>
  )
