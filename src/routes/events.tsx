...
import { Loading03Icon, AnalyticsUpIcon } from '@hugeicons/core-free-icons'

export function EventsPage() {
  const { competitions, selected, loading, searchTerm, setSearchTerm, toggle, selectAll, deselectAll } =
    usePastCompetitions()
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground">
          Analyze registration rates by event type across past competitions
        </p>
      </div>
          ...
          {analysis.length === 0 && !analyzing ? (
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Select competitions from the list and click "Analyze Events" to see registration rates
                </CardDescription>
              </CardHeader>
              <CardContent className="flex flex-col items-center gap-6 py-8">
                <div className="p-4 rounded-full bg-primary/10">
                  <HugeiconsIcon icon={AnalyticsUpIcon} className="size-12 text-primary" strokeWidth={1.5} />
                </div>
