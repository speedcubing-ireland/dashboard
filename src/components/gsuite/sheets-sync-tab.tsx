...
export function SheetsSyncTab() {
  ...
  const loadPreview = async () => {
    ...
    try {
      const id =
        spreadsheetId.match(/\/spreadsheets\/d\/([a-zA-Z0-9-_]+)/)?.[1] ||
        spreadsheetId;
      const { teams: lTeams, roles: lRoles } = await loadSheetData(
        accessToken,
        id,
      );
      setTeams(lTeams);
    ...
    try {
      ...
      affectedGroupEmails.forEach((groupEmail) => {
        queryClient.invalidateQueries({
          queryKey: groupKeys.detail(groupEmail),
        });
        queryClient.invalidateQueries({
          queryKey: ["gsuite", "groups", groupEmail, "members"],
        });
      });
              ...
              <Accordion type="single" collapsible className="w-full">
                <AccordionItem value="teams">
                  ...
                  <AccordionContent className="text-sm text-muted-foreground">
                    Columns: Team Name, Short Name, Leader Title, Contact,
                    Groups
                  </AccordionContent>
                </AccordionItem>
          ...
          <Tabs defaultValue="additions">
            ...
            <TabsContent value="additions">
              <ChangesTable
                ...
                emptyMsg="No members to add"
              />
            </TabsContent>
            <TabsContent value="removals">
              <ChangesTable
                changes={preview.removals}
                emptyMsg="No members to remove"
              />
            </TabsContent>
        ...
        <div className="grid gap-6">
          ...
          <Card>
            <CardContent className="grid gap-4 pt-6">
              <ConfigRow
                label="Add members"
                count={preview?.additions.length}
                checked={syncOpts.additions}
                onChange={(c) => setSyncOpts((s) => ({ ...s, additions: c }))}
              />
              <ConfigRow
                label="Update roles"
                count={preview?.updates.length}
                checked={syncOpts.updates}
                onChange={(c) => setSyncOpts((s) => ({ ...s, updates: c }))}
              />
              <ConfigRow
                label="Remove members"
                count={preview?.removals.length}
                checked={syncOpts.removals}
                onChange={(c) => setSyncOpts((s) => ({ ...s, removals: c }))}
                destructive
              />
            </CardContent>
            <CardFooter className="flex gap-4">
              ...
              <Button
                ...
                variant={syncOpts.removals ? "destructive" : "default"}
              >
                {isSyncing ? (
                  <>
                    <Spinner className="mr-2 h-4 w-4" /> Syncing...
                  </>
                ) : (
                  "Apply Changes"
                )}
        ...
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              <HugeiconsIcon
                icon={result.success ? CheckmarkCircle01Icon : AlertCircleIcon}
                className={`h-5 w-5 ${result.success ? "text-green-500" : "text-yellow-500"}`}
              />
              Sync Complete
            </CardTitle>
          </CardHeader>
