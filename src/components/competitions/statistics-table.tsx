    ...
    <div className="space-y-6">
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Upcoming Irish competitions and registration status
          </CardDescription>
        </CardHeader>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Number of rounds per event at each competition
          </CardDescription>
        </CardHeader>
        <CardContent className="overflow-x-auto">
          <Table>
            <TableHeader>
              <TableRow>
                ...
                {EVENT_COLS.map((e) => (
                  <TableHead key={e} className="text-center px-2 min-w-[4ch]">
                    {e.trim()}
                  </TableHead>
                ))}
              </TableRow>
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between space-y-0">
          <div className="space-y-1">
            ...
            <CardDescription>
              {showRawNumbers
                ? "Raw registration counts per event"
                : "Percentage of competitors per event"}
            </CardDescription>
          </div>
        ...
        <CardContent className="overflow-x-auto">
          <Table>
            ...
            <TableBody>
              {(showRawNumbers ? data.rawSignups : data.signups).map((row) => (
                <TableRow key={row.Name}>
                  <TableCell className="font-medium">{row.Name}</TableCell>
                  {EVENT_COLS.map((e) => (
                    <TableCell
